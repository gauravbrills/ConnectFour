...
public class A1Program {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (userInputs[i] >= 1 & userInputs[i] <= 7) {
                    ...
                    if (rowString.toString().trim().length() < 6) {
                        rowString = new StringBuilder(padWithWS(rowString.toString().trim() + token[i]));
                        input = mergeRowWithBoard(input, rowString.toString(), userInputs[i]);
    ...
    private String mergeRowWithBoard(String input, String rowString, Integer player1Input) {
        String out = input.substring(0, (player1Input - 1) * 6) + rowString + input.substring(player1Input * 6, 42);
        return out;
    }
    ...
    private Integer getInputFromPlayer(int i) {
        int chance = 0;
        System.out.println("Player " + (i + 1) + " Enter column for " + token[i]);
